...
const mockedImportantDatesPage: PageResponse = {
  id: 20,
...
const getMockedPage = ({ req }): PageResponse => ({
  id: 7,
  url: req.query?.url ?? '/test_page',
  pageName: 'test_page',
  title: req.query?.url ?? 'test_page',
...
export const pageMocks = {
  ...
  '/pages': {
    get: ({ req }) => ({
      prisma: {
        page: {
          findMany: async () => [getMockedPage({ req }), getMockedPage({ req: { query: { url: '/404' } } })],
        },
      },
    }),
  },
