...
export async function getServerSideProps({ res }) {
  ...
  const pageProps = await getPage('/utbildningsanordnare/kontakta', res);
  return await _.merge(layoutProps, pageProps);
}

export const Utbildningsanordnare: React.FC = ({ pageData, layoutData }: PageProps) => {
  return (
    <DefaultLayout title={`Yrkesutbildning - Utbildningsanordnare - Kontakta`} layoutData={layoutData}>
      <ContentBlock>
        <BigDropHeader
          ...
          breadcrumbs={
            <Breadcrumb className="" separator={<span className="mx-1">|</span>}>
              ...
              <Breadcrumb.Item>
                <NextLink href="/utbildningsanordnare/kontakta" passHref legacyBehavior>
                  <Breadcrumb.Link currentPage href="/utbildningsanordnare/kontakta">
